...
if __name__ == '__main__':
    ...
    for i in range(random_ind):
        ...
        labels = test_dataloader.dataset.label[i, ind].to(torch.long).detach().cpu()
        correct_point = preds_labels[i].eq(labels.data).cpu().sum().item()
        num_point = labels.view([-1,1]).size()[0]
        accuracy = correct_point / num_point
        print("accuracy is",accuracy)
        if(accuracy<0.7):
            print("failed sagmentation",i)
            viz_seg(verts, labels, "{}/2. fail_vis_{}_gt_{}_acc{}.gif".format(args.output_dir, i, args.exp_name, accuracy), args.device)
            viz_seg(verts, preds_labels[i], "{}/2. fail_vis_{}_pred_{}.gif".format(args.output_dir, i, args.exp_name), args.device)
        else:
            print("correct prediction",i)
            viz_seg(verts, labels, "{}/2. random_vis_{}_gt_{}_acc{}.gif".format(args.output_dir, i, args.exp_name, accuracy), args.device)
            viz_seg(verts, preds_labels[i], "{}/2. random_vis_{}_pred_{}.gif".format(args.output_dir, i, args.exp_name), args.device)
